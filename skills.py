...
def find_unique_common_items(list1, list2):
    ...
    common_unique_items = set(list1) & set(list2)
...
def get_sum_zero_pairs(input_list):
    ...
    for num in input_list:
        if -num in sum_pairs.keys():
            sum_pairs[-num] = num
        else:
            if num not in sum_pairs.keys():
                sum_pairs[num] = 0

    zero_pairs = {key: value for key, value in sum_pairs.iteritems()
                  if key + value == 0}

    return zero_pairs.items()
...
def sort_by_word_length(words):
    ...
    for word in words:
        ...
        if len(word) not in sorted_word_length.keys():
            sorted_word_length[len(word)] = [word]
        else:
            sorted_word_length[len(word)] += [word]

    return sorted_word_length.items()
...
def get_pirate_talk(phrase):
    ...
    for word in words:
        if word in pirate_code:
            pirate_word = pirate_code[word]
            pirate_message.append(pirate_word)
        else:
            pirate_message.append(word)

    return " ".join(pirate_message)
